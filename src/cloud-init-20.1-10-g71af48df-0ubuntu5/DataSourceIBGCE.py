...
# Only metadata_url is changed and "GCE" is renamed to "IBGCE" comparing
# to original GCE datasoure.
# Code reference changes inherited from the link
# https://github.com/canonical/cloud-init/blob/main/cloudinit/sources/DataSourceGCE.py

# 169.254.169.254 is used instead of original "metadata.google.internal"
# because we don't have dns resolver enabled and metadata server is
# always at 169.254.169.254
#MD_V1_URL = 'http://metadata.google.internal/computeMetadata/v1/'
MD_V1_URL = 'http://169.254.169.254/computeMetadata/v1/'
IB_BUILTIN_DS_CONFIG = {'metadata_url': MD_V1_URL}
REQUIRED_FIELDS = ('instance-id', 'availability-zone', 'local-hostname')
GUEST_ATTRIBUTES_URL = ('http://169.254.169.254/computeMetadata/'
...
class DataSourceIBGCE(sources.DataSource):
    ...
    def __init__(self, sys_cfg, distro, paths):
        ...
        self.ds_cfg = util.mergemanydict([
            util.get_cfg_by_path(sys_cfg, ["datasource", "IBGCE"], {}),
            IB_BUILTIN_DS_CONFIG])
        self.metadata_address = self.ds_cfg['metadata_url']

    def _get_data(self):
        ret = util.log_time(
            LOG.debug, 'Crawl of IBGCE metadata service',
...
def read_md(address=None, platform_check=True):
    ...
    for (mkey, paths, required, is_text, is_recursive) in url_map:
        ...
        if required and value is None:
            msg = "required key %s returned nothing. not IBGCE"
            ret['reason'] = msg % mkey
            return ret
        md[mkey] = value
...
datasources = [
    (DataSourceIBGCE, (sources.DEP_FILESYSTEM, sources.DEP_NETWORK)),
]
