...
IB_DS_NAME = 'IBAzure'
IB_DEFAULT_METADATA = {"instance-id": "iid-AZURE-NODE"}
IB_AGENT_START = ['service', 'walinuxagent', 'start']
IB_BOUNCE_COMMAND = ['sh', '-xc',
    "i=$interface; x=0; ifdown $i || x=$?; ifup $i || x=$?; exit $x"]
IB_DATA_DIR_CLEAN_LIST = ['SharedConfig.xml']

IB_BUILTIN_DS_CONFIG = {
    'agent_command': IB_AGENT_START,
    'data_dir': "/var/lib/waagent",
    'set_hostname': True,
    'hostname_bounce': {
        'interface': 'eth0',
        'policy': True,
        'command': IB_BOUNCE_COMMAND,
        'hostname_command': 'hostname',
        },
    'disk_aliases': {'ephemeral0': '/dev/sdb'},
}

IB_BUILTIN_CLOUD_CONFIG = {
    'disk_setup': {
...
IB_DS_CFG_PATH = ['datasource', IB_DS_NAME]
IB_DEF_EPHEMERAL_LABEL = 'Temporary Storage'


class DataSourceIBAzureNet(sources.DataSource):
    def __init__(self, sys_cfg, distro, paths):
        ...
        self.ds_cfg = util.mergemanydict([
            util.get_cfg_by_path(sys_cfg, IB_DS_CFG_PATH, {}),
            IB_BUILTIN_DS_CONFIG])
    ...
    def get_data(self):
        ...
        if found != ddir:
            ...
            if cached_ovfenv != files['ovf-env.xml']:
                ...
                cleaned = []
                for f in [os.path.join(ddir, f) for f in IB_DATA_DIR_CLEAN_LIST]:
                    if os.path.exists(f):
                        util.del_file(f)
                        cleaned.append(f)
                if cleaned:
...
def find_ephemeral_part():
    ...
    that has a LABEL of IB_DEF_EPHEMERAL_LABEL and is a NTFS device. If Azure
    gets more ephemeral devices, this logic will only identify the first
    such device.
    """
    c_label_devs = util.find_devs_with("LABEL=%s" % IB_DEF_EPHEMERAL_LABEL)
    ...
    for child in lpcs.childNodes:
        ...
        if name == "userdata" or name == "customdata":
            ...
            if attrs.get('encoding') in (None, "base64"):
                ...
                dscfg = value
            cfg['datasource'] = {IB_DS_NAME: util.load_yaml(dscfg, default={})}
        elif name == "ssh":
...
datasources = [
  (DataSourceIBAzureNet, (sources.DEP_FILESYSTEM, sources.DEP_NETWORK)),
]
