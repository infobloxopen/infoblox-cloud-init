...
VNIC_ENDPOINT = "http://169.254.169.254/opc/v1/vnics"

class DataSourceIBOracle(sources.DataSource):
    ...
    def crawl_vnics_metadata(self):
        return read_vnics_metadata()
...
def read_vnics_metadata(endpoint_base=VNIC_ENDPOINT, sys_uuid=None):
    """ Read vnics metadata and configure the public interface

    The vnic metadata returns string of total configured vnics
    The second string contains the public interface
    """
    endpoint = combine_url(endpoint_base) + "/"
    resp = readurl(endpoint)
    vnics = (json.loads(resp.contents.decode('utf-8')))

    ip_address = vnics[1].get("privateIp")
    subnet_mask_cidr = vnics[1].get("subnetCidrBlock")
    gateway =  vnics[1].get("virtualRouterIp")

    lan_ip_address = [ip_address, subnet_mask_cidr, gateway]

    filename = "/infoblox/var/.oci_lan_ip"

    for content in lan_ip_address:
        try:
...
datasources = [
    (DataSourceIBOracle, (sources.DEP_FILESYSTEM, sources.DEP_NETWORK)),
]
...
if __name__ == "__main__":
    ...
    read_vnics_metadata(endpoint_base=VNIC_ENDPOINT, sys_uuid=sys_uuid)
